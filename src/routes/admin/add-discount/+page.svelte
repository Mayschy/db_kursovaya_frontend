<script lang="ts">
	...
	async function fetchProducts(query: string, page: number) {
		const elements = (await PrivateMarketplaceApi.findProducts(query, page, 15)).data
			.content as any[];
	...
	async function fetchCategories(query: string, page: number) {
		const elements = (await PrivateMarketplaceApi.findCategory(query, page, 15)).data
			.content as any[];
			...
			<div class="mb-3">
				<Selector fetchElements={fetchCategories} let:item>
					...
					<AddRemoveItem
						{item}
						conatains={(item) => {
							return byCategoryRequest.targetCategoriesIds.includes(item.name);
						}}
						handler={(item, add) => {
							if (add) byCategoryRequest.targetCategoriesIds.push(item.name);
							else {
								const index = byCategoryRequest.targetCategoriesIds.indexOf(item.name);
